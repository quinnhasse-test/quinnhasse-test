...
Transparent AI answer system built at MadHacks 2025. Constrains GPT-4o-mini to retrieved sources only (via Exa neural search), then constructs a 4-layer evidence graph linking every claim back to its source. Interactive 3D visualization built with React Three Fiber. Deployed at [lexon-ai.vercel.app](https://lexon-ai.vercel.app/).

### [RealTimeASLTranslator](https://github.com/quinnhasse-test/RealTimeASLTranslator)
Real-time American Sign Language hand gesture classifier. OpenCV handles frame capture and preprocessing at 30 fps; a CNN trained on ASL gesture images outputs letter predictions as a live overlay. Built with TensorFlow.
